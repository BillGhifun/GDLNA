...
Web 界面中每条媒体记录提供以下操作按钮：

| 按钮 | 说明 |
|------|------|
| **播放** | 直接在线播放媒体，使用内置播放器打开媒体链接 |
| **保存** | 程序自动将媒体下载到**程序所部署的设备**中（服务器端下载），保存路径由程序配置决定 |
| **下载** | 将媒体下载到**当前打开前端页面的设备**中（客户端下载），即用户正在使用的手机/电脑 |
| **复制** | 复制媒体链接到剪贴板，可用于在别处打开或进行 URL 参数分析 |
